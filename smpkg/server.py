...
from database.maintenance_personnel import MaintenanceWorker, Capacity, MaintenanceRecord, MaintenancePerformance, CapacityRate
...
from smpkg.database_utils import (EntityQueryByAtt, RelQueryByEnt, getRelEnt, CreateEnt,
    handle_time_key, CreateRel, UpdateEnt, UpdateRel, DeleteEnt, DeleteRel, RelQueryByEntsAttr)
from smpkg.prompt import InformationExtractionArrayPrompt, PromptDateFormatConstraint

app = FastAPI()
local_llm_controller = None
...
database_modules = [ sys.modules[key] for key in sys.modules if key.startswith(database_module_name + '.') and key != database_module_name ]
...
[ kg_mapping.update({ key: value for key, value in dic.items() if issubclass(value, StructuredNode) and value != StructuredNode })
    for dic in database_dics ]

kg_rel_mapping: dict[str, type[StructuredRel]] = dict()
[ kg_rel_mapping.update({ key: value for key, value in dic.items() if issubclass(value, StructuredRel) and value != StructuredRel })
    for dic in database_dics ]
    ...
    try:
        if key == "capacity":
            try:
                ...
                cap: list[Capacity] = Capacity.nodes.filter(name=data)
            except Exception as e:
